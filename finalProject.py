...
# 'This page will show the top ten lists for owner #
...
def editList(owner_id, tlist_id):
    ...
    if request.method == 'POST':
        ...
        list_to_edit = session.query(Lists).filter_by(id=tlist_id).one()
        return render_template('editList.html', owner=owner_of_list,
                               tlist=list_to_edit)
...
# 'This page will show the top ten items for a list '#
...
def topTenItems(owner_id, tlist_id):
    ...
    if lists == []:
        ...
        return render_template('topTenItems.html', owner=owner_of_list,
                               tlist=tlist, items=items, owner_id=owner_id,
                               tlist_id=tlist_id, login_session=login_session)
...
def createNewItem(owner_id, tlist_id):
    ...
    if request.method == 'POST':
        ...
        owner = session.query(Owner).filter_by(id=owner_id).one()
        tlist = session.query(Lists).filter_by(id=tlist_id).one()
        return render_template('createNewItem.html', owner=owner,
                               tlist=tlist, owner_id=owner_id,
                               tlist_id=tlist_id)
...
def editItem(owner_id, tlist_id, item_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editItem.html', tlist=list_item_belongsto,
                               item=item_to_edit)
...
def deleteItem(owner_id, tlist_id, item_id):
    ...
    if request.method == 'POST':
        ...
        tlist = session.query(Lists).filter_by(id=tlist_id).one()
        item_to_delete = session.query(Items).filter_by(id=item_id).one()
        return render_template('deleteItem.html', owner=owner,
                               tlist=tlist, owner_id=owner_id,
                               tlist_id=tlist_id, item_id=item_id,
                               item=item_to_delete)
